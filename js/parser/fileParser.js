                    ...
                    if (checkExists(item2.$.ID, item.type)) return null;
                        ...
                        if (item2.input_port) {
                            item2.input_port.forEach(port => {
                                result.port[port.$.name] = { direction: 'input_port', dataType: port.$.type || port.$.dataType, defaultValue: port.$.default || '', description: port._ || '' }
                            });
                        }
                        ...
                        if (item2.output_port) {
                            item2.output_port.forEach(port => {
                                result.port[port.$.name] = { direction: 'output_port', dataType: port.$.type || port.$.dataType, defaultValue: port.$.default || '', description: port._ || '' }
                            });
                        }
                        ...
                        if (item2.inout_port) {
                            item2.inout_port.forEach(port => {
                                result.port[port.$.name] = { direction: 'inout_port', dataType: port.$.type || port.$.dataType, defaultValue: port.$.default || '', description: port._ || '' };
                            });
                        }
                        return result;
                    }
